...
export const useUpload = (directory?: string) => {
  ...
  const httpRequest = async (options: UploadRequestOptions) => {
    console.log('🚀 ~ httpRequest ~ options:', options)
    // 模式一：前端上传
    if (isClientUpload) {
      // 1.1 生成文件名称
      ...
      if (options.data.imgType == 2) {
        return new Promise((resolve, reject) => {
          FileApi.updateFileT({
            file: options.file,
            directory,
            bucketName: options.data.bucketName
          })
            .then((res) => {
              if (res.code === 0) {
                resolve(res)
        ...
        return new Promise((resolve, reject) => {
          FileApi.updateFile({ file: options.file, directory, bucketName: options.data.bucketName })
            .then((res) => {
              if (res.code === 0) {
                resolve(res)
              } else {
