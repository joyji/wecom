 ...
 * @LastEditors: jiying
 * @LastEditTime: 2025-06-26 16:36:05
 */
...
// 获取oneId code
export const getOneidCode = () => {
  return wsCache.get(OneidCodeKey) ? wsCache.get(OneidCodeKey) : wsCache.get('oneid_code')
}
// TODO: oneID 设置token
export const setOneIdToken = (token: TokenType, code: string) => {
  wsCache.set(RefreshTokenKey, token.refreshToken)
  wsCache.set(AccessTokenKey, token.accessToken)
  wsCache.set(UserIdKey, token.userId)
  // 设置oneId code
  wsCache.set(OneidCodeKey, code)
}
