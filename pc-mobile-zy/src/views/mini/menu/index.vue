...
import * as MenuApi from '@/api/mini/menu'
import { MenuVO } from '@/api/mini/menu'
...
const columns = [
  ...
  },
  {
    ...
    cellRenderer: ({ rowData }) => {
      ...
      if (checkPermi(['system:menu:update'])) {
        buttons.push(
          <ElButton key="edit" link type="primary" onClick={() => openForm('update', rowData)}>
            修改
          </ElButton>
        )
      }
      if (checkPermi(['system:menu:create'])) {
        buttons.push(
          <ElButton
            ...
            link
            type="primary"
            onClick={() => openForm('create', undefined, rowData)}
          >
            新增
          </ElButton>
        )
...
/** 添加/修改操作 */
const formRef = ref()
const openForm = (type: string, obj?: Object, parentId?: number) => {
  formRef.value.open(type, obj, parentId)
}
...
const handleStatusChanged = async (menu: any, val: number) => {
  // 1. 标记 menu.id 更新中
